...
function dhtmlXTabBar(parentObject, mode, height) {
	...
	this._s.line_height = (parseInt(height) || 40);
	this._s.skin_line = 1;
...
dhtmlXTabBar.prototype = {
	...
	_setSizes: function(x, y) {
		...
		this._tabZone.style[this._dy] = _t +2+ "px";
		...
		this._tabZone.style[this._py] = "0";
		this._conZone.style[this._py] = "43px";
		this._checkScroll();
	...
	_showScroll: function(i) {
		...
		a[0].style.cssText = "background-image:url(" + this.imgUrl + this.skin + "/" + this.skin + "_scroll_left.gif);width:40px;height:30px;position:absolute;left:0px;z-index:990;top:" + top + "px;";
		a[0].className = "dhx_tab_scroll_left";
		this._rows[i].appendChild(a[0]);
		a[1] = document.createElement("DIV");
		a[1].style.cssText = "background-image:url(" + this.imgUrl + this.skin + "/" + this.skin + "_scroll_right.gif);width:40px;height:21px;position:absolute;right:0px;z-index:990;top:" + top + "px;";
	...
	_createTab: function(id, text, size) {
		...
		var thml = "<span style=" + (this.skin.indexOf("dhx_sky") == 0 ? ("line-height:" + this._s.line_height + "px;") : "") + ">" + text + "</span><div style='display:none;'></div><div style='display:none;'></div><div style='display:none;'></div>";
		...
		return tab
	}, //tab.childNodes[3].style.backgroundPositionY=-97;
	...
	adjustTabSize: function(tab, size) {
		...
		if(!size) {
			...
			size = tab.scrollWidth + 10 + (this._close ? 40 : 0);
	...
	_getCoverLine: function() {
		if(!this._lineA) {
			...
			this._lineA.style[this._py] = this._s.line_height - (this._s.skin_line_x ? 1 : 0) + "px";
			this._lineA.style[this._dx] = "100%";
			this._lineA._index = 1
		};
		this._lineA.style.zIndex = (this._lineA._index++);
		/*this._lineA.style.zIndex = "-1";*/
		return this._lineA
	},
	cells: function(id) {
		...
		if(!this._content[id]) {
			...
			d.show = function() {
				...
				if(self._awdj || self._ahdj) {
					...
					if(self._ahdj) self._tabAll.style.height = self.entBox.style.height = dim[1] + self._rows.length * (self._s.line_height - 2) + (self._s.expand ? 0 : 2) + 2 + "px";
					self._setSizes();
