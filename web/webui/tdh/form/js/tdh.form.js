...
function createOptions($sel){
	var $wrap=$sel.prev(),
		...
		$list=jq('<ul class="'+l+'" data-sel="'+$wrap.attr('data-index')+'" style="width:'+$wrap.outerWidth()+'px;z-index:99999999;"></ul>').appendTo('body');
	var liHtml='';
...
function setListHeight($div,$list){
	...
	jq(window,document).not($list).on(eventType, function(event) {
