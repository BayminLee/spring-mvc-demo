...
$(function(){
	...
	$('#capnav').find('a').live('click',function(){
		...
		$('#contFrame').attr('src',$(this).attr('data-url'));
		capActiveId=$(this).attr('data-id');
	});
	//标签移动
...
function addCapnav(objid,objdata){
	...
	$('#contFrame').attr('src',objdata.url);
}
//标签移动按钮显示隐藏
...
function delCapnav(objid){
	...
	if(capActiveId==objid){
		...
		$('#contFrame').attr('src',$('#capnav').find('a[data-id="'+capActiveId+'"]').attr('data-url'));
	}
	wrap.remove();
	//标签样式
...
function delTool(type){
	...
	$('#contFrame').attr('src',capActiveObj.attr('data-url'));
}